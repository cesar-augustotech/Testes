...
import stardew.interfaces.NivelUpObserver;
...
public class Habilidades {
    ...
    /** Observer Pattern — lista de observadores de nível up. */
    private final List<NivelUpObserver> observers = new ArrayList<>();

    public void addObserver(NivelUpObserver obs) {
        observers.add(obs);
    }
     ...
     * Se o nível subir, notifica todos os NivelUpObserver registrados.
     * @return true se o nível subiu.
     */
    public boolean ganharXp(Tipo tipo, int xp) {
        boolean subiu = switch (tipo) {
        ...
        // Observer Pattern: notifica UI (e outros) sem acoplar Habilidades ao GameEngine
        if (subiu) {
            int novoNivel = getNivel(tipo);
            observers.forEach(o -> o.onNivelUp(tipo, novoNivel));
        }
        return subiu;
    }
